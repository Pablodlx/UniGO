...
def cancel_auto_bookings_for_dates(
    ...
    SOLO cancela bookings que fueron generados por esta alerta específica (search_alert_id == alert.id)
    y que corresponden a los días eliminados. NO cancela bookings manuales ni de otras alertas.
    
    Args:
        db: Database session
        alert: The SearchAlert object
        removed_dates: List of date objects that were removed from the alert
        alert_origin: Origin of the alert (unused, kept for compatibility)
        alert_destination: Destination of the alert (unused, kept for compatibility)
        alert_target_time: Target time of the alert (unused, kept for compatibility)
        alert_flexibility_minutes: Flexibility in minutes (unused, kept for compatibility)
    """
    ...
    # Convert removed_dates to set for faster lookup
    removed_dates_set = set(removed_dates)
    
    # Find bookings that were generated by this specific alert and are in the removed dates
    # IMPORTANTE: Solo cancelar bookings que tienen search_alert_id == alert.id
    bookings_to_cancel = db.query(Booking).join(Ride).filter(
        Booking.search_alert_id == alert.id,  # SOLO bookings generados por esta alerta
        Booking.status.in_([BookingStatus.pending, BookingStatus.confirmed]),
        Ride.is_active == True,
    ).all()
    ...
    for booking in bookings_to_cancel:
        ...
        if ride_date not in removed_dates_set:
            continue
        
        # This booking was generated by this alert and is for a removed date - cancel it
        try:
            ...
            canceled_count += 1
            print(f"[ALERT UPDATE] Canceled auto-booking {booking.id} for removed date {ride_date} (alert {alert.id})")
            
        except Exception as e:
            print(f"[ALERT UPDATE] Error canceling booking {booking.id}: {e}")
            import traceback
            traceback.print_exc()
            continue
    
    if canceled_count > 0:
        print(f"[ALERT UPDATE] ✅ Canceled {canceled_count} auto-bookings for removed dates from alert {alert.id}")
