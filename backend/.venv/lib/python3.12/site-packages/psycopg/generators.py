...
def _connect(conninfo: str, *, timeout: float = 0.0) -> PQGenConn[PGconn]:
    ...
    while True:
        ...
        if status == POLL_READING or status == POLL_WRITING:
            ...
            raise e.InternalError(f"unexpected poll status: {status}", pgconn=e.finish_pgconn(conn))
...
def _cancel(cancel_conn: PGcancelConn, *, timeout: float = 0.0) -> PQGenConn[None]:
    ...
    while True:
        ...
        if status == POLL_OK:
            ...
            raise e.OperationalError(f"cancellation failed: {cancel_conn.get_error_message()}")
        else:
