...
class Transformer(AdaptContext):
    ...
    def set_pgresult(
        ...
        *,
        ...
        self._row_loaders = [self.get_loader(result.ftype(i), fmt).load for i in range(nf)]
    ...
    def load_rows(self, row0: int, row1: int, make_row: RowMaker[Row]) -> list[Row]:
        ...
        if not (0 <= row0 <= self._ntuples and 0 <= row1 <= self._ntuples):
            raise e.InterfaceError(f"rows must be included between 0 and {self._ntuples}")
    ...
    def load_sequence(self, record: Sequence[Buffer | None]) -> tuple[Any, ...]:
        ...
        return tuple(
            (self._row_loaders[i](val) if val is not None else None) for i, val in enumerate(record)
        )
