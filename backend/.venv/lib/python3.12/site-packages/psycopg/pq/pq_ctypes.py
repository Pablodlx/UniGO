...
class PGconn:
    ...
    def send_query_params(
        ...
        if not impl.PQsendQueryParams(*args):
            raise e.OperationalError(f"sending query and params failed: {self.get_error_message()}")
    ...
    def send_query_prepared(
        ...
        args = self._query_params_args(name, param_values, None, param_formats, result_format)
        args = args[:3] + args[4:]

        self._ensure_pgconn()
        if not impl.PQsendQueryPrepared(*args):
            raise e.OperationalError(f"sending prepared query failed: {self.get_error_message()}")
    ...
    def _query_params_args(
        ...
        if not param_types:
            ...
            if len(param_types) != nparams:
                raise ValueError(
                    "got %d param_values but %d param_types" % (nparams, len(param_types))
                )
            atypes = (impl.Oid * nparams)(*param_types)
        ...
        if not param_formats:
            ...
            if len(param_formats) != nparams:
                raise ValueError(
                    "got %d param_values but %d param_formats" % (nparams, len(param_formats))
                )
    ...
    def exec_prepared(
        ...
        if not param_formats:
            ...
            if len(param_formats) != nparams:
                raise ValueError(
                    "got %d param_values but %d param_types" % (nparams, len(param_formats))
                )
            aformats = (c_int * nparams)(*param_formats)
        ...
        if not rv:
            raise e.OperationalError(f"executing prepared query failed: {self.get_error_message()}")
        return PGresult(rv)
    ...
    def describe_prepared(self, name: bytes) -> PGresult:
        ...
        if not rv:
            raise e.OperationalError(f"describe prepared failed: {self.get_error_message()}")
        return PGresult(rv)
    ...
    def describe_portal(self, name: bytes) -> PGresult:
        ...
        if not rv:
            raise e.OperationalError(f"describe portal failed: {self.get_error_message()}")
        return PGresult(rv)
    ...
    def send_describe_portal(self, name: bytes) -> None:
        ...
        if not impl.PQsendDescribePortal(self._pgconn_ptr, name):
            raise e.OperationalError(f"sending describe portal failed: {self.get_error_message()}")
    ...
    def close_prepared(self, name: bytes) -> PGresult:
        ...
        if not rv:
            raise e.OperationalError(f"close prepared failed: {self.get_error_message()}")
        return PGresult(rv)
    ...
    def send_close_prepared(self, name: bytes) -> None:
        ...
        if not impl.PQsendClosePrepared(self._pgconn_ptr, name):
            raise e.OperationalError(f"sending close prepared failed: {self.get_error_message()}")
    ...
    def send_close_portal(self, name: bytes) -> None:
        ...
        if not impl.PQsendClosePortal(self._pgconn_ptr, name):
            raise e.OperationalError(f"sending close portal failed: {self.get_error_message()}")
    ...
    def encrypt_password(self, passwd: bytes, user: bytes, algorithm: bytes | None = None) -> bytes:
        """
        ...
        if not out:
            raise e.OperationalError(f"password encryption failed: {self.get_error_message()}")
...
class PGcancelConn:
    ...
    def start(self) -> None:
        ...
        if not impl.PQcancelStart(self.pgcancelconn_ptr):
            raise e.OperationalError(f"couldn't start cancellation: {self.get_error_message()}")
...
class PGcancel:
    ...
    def cancel(self) -> None:
        ...
        if not res:
            raise e.OperationalError(f"cancel failed: {buf.value.decode('utf8', 'ignore')}")
    ...
    def parse(cls, conninfo: bytes) -> list[ConninfoOption]:
        ...
        if not rv:
            if not errmsg:
                ...
                exc = e.OperationalError((errmsg.value or b"").decode("utf8", "replace"))
...
class Escaping:
    ...
    def escape_bytea(self, data: abc.Buffer) -> bytes:
        ...
        if not out:
            raise MemoryError(f"couldn't allocate for escape_bytea of {len(data)} bytes")
    ...
    def unescape_bytea(self, data: abc.Buffer) -> bytes:
        ...
        if not out:
            raise MemoryError(f"couldn't allocate for unescape_bytea of {len(data)} bytes")
