...
class BaseCursor(Generic[ConnectionType, Row]):
    ...
    def _stream_fetchone_gen(self, first: bool) -> PQGen[PGresult | None]:
        ...
        if status == SINGLE_TUPLE or status == TUPLES_CHUNK:
            ...
            while res:
                res = yield from fetch(self._pgconn)
            if status != TUPLES_OK:
                raise e.ProgrammingError("the operation in stream() didn't produce a result")
            return None
    ...
    def _start_copy_gen(self, statement: Query, params: Params | None = None) -> PQGen[None]:
    ...
    def _convert_query(self, query: Query, params: Params | None = None) -> PostgresQuery:
    ...
    def _raise_for_result(self, result: PGresult) -> NoReturn:
        ...
        if status == FATAL_ERROR:
            ...
            raise e.ProgrammingError("COPY cannot be used with this method; use copy() instead")
        else:
