...
_unpack_oidlen = cast(Callable[[abc.Buffer, int], "tuple[int, int]"], _struct_oidlen.unpack_from)
...
class SequenceDumper(RecursiveDumper):
    def _dump_sequence(self, obj: Sequence[Any], start: bytes, end: bytes, sep: bytes) -> bytes:
        if not obj:
            return start + end
...
class RecordLoader(BaseCompositeLoader):
    def load(self, data: abc.Buffer) -> tuple[Any, ...]:
        ...
        return tuple(
            cast(token) if token is not None else None for token in self._parse_record(data[1:-1])
        )
...
class CompositeLoader(RecordLoader):
    ...
    def load(self, data: abc.Buffer) -> Any:
        ...
        return type(self).factory(*self._tx.load_sequence(tuple(self._parse_record(data[1:-1]))))
...
def _make_binary_loader(name: str, factory: Callable[..., Any]) -> type[BaseCompositeLoader]:
    return type(f"{name.title()}BinaryLoader", (CompositeBinaryLoader,), {"factory": factory})
