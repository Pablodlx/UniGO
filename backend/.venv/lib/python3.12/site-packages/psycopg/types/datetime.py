...
_unpack_interval = cast(Callable[[Buffer], "tuple[int, int, int]"], _struct_interval.unpack)
...
class _BaseTimeDumper(Dumper):
    ...
    def _get_offset(self, obj: time) -> timedelta:
        ...
        if offset is None:
            raise DataError(f"cannot calculate the offset of tzinfo '{obj.tzinfo}' without a date")
        return offset
...
class TimeBinaryDumper(_BaseTimeDumper):
    ...
    def dump(self, obj: time) -> Buffer | None:
        us = obj.microsecond + 1_000_000 * (obj.second + 60 * (obj.minute + 60 * obj.hour))
        return pack_int8(us)
...
class TimestamptzLoader(Loader):
    ...
    def _load_notimpl(self: TimestamptzLoader, data: Buffer) -> datetime:
        ...
        raise NotImplementedError(f"can't parse timestamptz with DateStyle {ds!r}: {s!r}")
...
class TimestamptzBinaryLoader(Loader):
    ...
    def load(self, data: Buffer) -> datetime:
        ...
        try:
            ...
            if self._timezone:
                utcoff = self._timezone.utcoffset(datetime.min if micros < 0 else datetime.max)
                if utcoff:
...
class IntervalLoader(Loader):
    ...
    def _load_notimpl(self: IntervalLoader, data: Buffer) -> timedelta:
        ...
        raise NotImplementedError(f"can't parse interval with IntervalStyle {ints}: {s!r}")
...
_month_abbr = {
    n: i for i, n in enumerate(b"Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec".split(), 1)
}
