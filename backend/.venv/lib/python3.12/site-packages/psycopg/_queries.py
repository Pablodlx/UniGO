...
class PostgresQuery:
    ...
    def is_params_sequence(vars: Params) -> TypeGuard[Sequence[Any]]:
        ...
        if t is list or t is tuple:
            ...
            raise TypeError(
                "query parameters should be a sequence or a mapping," f" got {type(vars).__name__}"
            )
        return sequence
    ...
    def validate_and_reorder_params(
        ...
        if PostgresQuery.is_params_sequence(vars):
            ...
            if vars and len(parts) > 1 and not isinstance(parts[0][1], str):
                raise TypeError("positional placeholders (%s) require a sequence of parameters")
            try:
                if order:
...
def _split_query(
    ...
    while i < len(parts):
        ...
        if ph == b"%(":
            raise e.ProgrammingError(
                "incomplete placeholder:" f" '{query[m.span(0)[0]:].split()[0].decode(encoding)}'"
            )
        elif ph == b"% ":
