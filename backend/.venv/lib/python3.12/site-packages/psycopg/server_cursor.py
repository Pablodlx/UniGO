...
class ServerCursorMixin(BaseCursor[ConnectionType, Row]):
    ...
    def _close_gen(self) -> PQGen[None]:
        ...
        if not self._described:
            query = sql.SQL("SELECT 1 FROM pg_catalog.pg_cursors WHERE name = {}").format(
                sql.Literal(self._name)
            )
...
class AsyncServerCursor(ServerCursorMixin["AsyncConnection[Any]", Row], AsyncCursor[Row]):
    ...
    def __init__(
        ...
        *,
        ...
        AsyncCursor.__init__(self, connection, row_factory=row_factory or connection.row_factory)
        ServerCursorMixin.__init__(self, name, scrollable, withhold)
