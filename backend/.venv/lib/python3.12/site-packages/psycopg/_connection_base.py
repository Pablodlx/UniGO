...
class BaseConnection(Generic[Row]):
    ...
    @staticmethod
    def _notice_handler(wself: ReferenceType[BaseConnection[Row]], res: PGresult) -> None:
        self = wself()
    ...
    @staticmethod
    def _notify_handler(wself: ReferenceType[BaseConnection[Row]], pgn: pq.PGnotify) -> None:
        self = wself()
    ...
    @classmethod
    def _connect_gen(cls, conninfo: str = "", *, timeout: float = 0.0) -> PQGenConn[Self]:
    ...
    def _check_connection_ok(self) -> None:
        ...
        raise e.InterfaceError(
            "cannot execute operations: the connection is" f" in status {self.pgconn.status}"
        )
    ...
    def _commit_gen(self) -> PQGen[None]:
        ...
        if self._tpc:
            raise e.ProgrammingError("commit() cannot be used during a two-phase transaction")
        if self.pgconn.transaction_status == IDLE:
            return
    ...
    def _rollback_gen(self) -> PQGen[None]:
        ...
        if self._tpc:
            raise e.ProgrammingError("rollback() cannot be used during a two-phase transaction")
    ...
    def _tpc_begin_gen(self, xid: Xid | str) -> PQGen[None]:
        ...
        if self._autocommit:
            raise e.ProgrammingError("can't use two-phase transactions in autocommit mode")
    ...
    def _tpc_finish_gen(self, action: LiteralString, xid: Xid | str | None) -> PQGen[None]:
        fname = f"tpc_{action.lower()}()"
        if xid is None:
            if not self._tpc:
                raise e.ProgrammingError(
                    f"{fname} without xid must must be" " called inside a two-phase transaction"
                )
            xid = self._tpc[0]
        else:
            if self._tpc:
                raise e.ProgrammingError(
                    f"{fname} with xid must must be called" " outside a two-phase transaction"
                )
            if not isinstance(xid, Xid):
                xid = Xid.from_string(xid)
        ...
        if self._tpc and not self._tpc[1]:
            ...
            yield from self._exec_command(SQL("{} PREPARED {}").format(SQL(action), str(xid)))
            self._tpc = None
