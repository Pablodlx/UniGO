...
if hasattr(selectors, "EpollSelector"):
    _epoll_evmasks = {
        ...
        WAIT_RW: select.EPOLLONESHOT | (select.EPOLLIN | select.EPOLLOUT | select.EPOLLERR),
    }
else:
    _epoll_evmasks = {}
...
if "PSYCOPG_WAIT_FUNC" in os.environ:
    ...
    if not fname.startswith("wait_") or fname not in globals():
        raise ImportError(
            "PSYCOPG_WAIT_FUNC should be the name of an available wait function;" f" got {fname!r}"
        )
    wait = globals()[fname]
