...
try:
    ...
    raise ImportError("the module psycopg._dns requires the package 'dnspython' installed")
...
class Rfc2782Resolver:
    ...
    def _get_solved_entries(self, hp: HostPort, entries: Sequence[SRV]) -> list[HostPort]:
        if not entries:
    ...
    def _return_params(self, params: dict[str, Any], hps: list[HostPort]) -> dict[str, Any]:
        if not hps:
