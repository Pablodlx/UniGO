...
class TypeInfo:
    ...
    @overload
    @classmethod
    def fetch(cls: type[T], conn: Connection[Any], name: str | sql.Identifier) -> T | None: ...
    ...
    @classmethod
    def fetch(cls: type[T], conn: BaseConnection[Any], name: str | sql.Identifier) -> Any:
        ...
        if isinstance(conn, Connection):
            ...
            raise TypeError(f"expected Connection or AsyncConnection, got {type(conn).__name__}")
    ...
    @classmethod
    async def _fetch_async(cls: type[T], conn: AsyncConnection[Any], name: str) -> T | None:
        try:
    ...
    @classmethod
    def _from_records(cls: type[T], name: str, recs: Sequence[dict[str, Any]]) -> T | None:
        if len(recs) == 1:
            return cls(**recs[0])
        elif not recs:
            return None
